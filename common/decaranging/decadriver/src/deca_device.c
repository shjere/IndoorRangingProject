...
/*
 * Wire up the user-provided SPI and Sleep functions
 */

void dwt_wire_spifcns(
...
{
    writetospi = spi_send_packet;
    readfromspi = spi_read_packet;
}

void dwt_wire_sleepfcns(
            void (*busy_sleep_msec)(uint32_t msec)
        )
{
    Sleep = busy_sleep_msec;
}
...
int dwt_initialise(uint16_t config)
{
    ...
    dw1000local.dwt_txcallback = 0 ;
    dw1000local.dwt_rxcallback = 0 ;
...
//    configure PLL2/RF PLL block CFG
    ...
    dwt_writetodevice(RF_CONF_ID, RF_RXCTRLH_OFFSET, 1, &rx_calibration[bw]);

    // Configure RF TX blocks (for specified channel and prf)
    //Config RF TX control
    dwt_write32bitoffsetreg(RF_CONF_ID, RF_TXCTRL_OFFSET, tx_calibration[chan_idx[chan]]);
 ...
 *
 * output parameters
 * uint32_trxtimestamp (hi 32-bits of RX timestamp)
 */
#pragma GCC optimize ("O3")
 ...
 *
 * output parameters
 * uint32_trxtimestamp (lo 32-bits of RX timestamp)
 */
#pragma GCC optimize ("O3")
 ...
 *      DWT_FF_NOTYPE_EN        0x000   no frame types allowed // PDM: they mean ALL!
...
void dwt_isr(void) // assume interrupt can supply context
{
	...
	if((status & (SYS_STATUS_LDEDONE | SYS_STATUS_RXPHD | SYS_STATUS_RXSFDD)) != (SYS_STATUS_LDEDONE | SYS_STATUS_RXPHD | SYS_STATUS_RXSFDD))
	{
		...
		else
		{
			...
			if(dw1000local.dwt_rxcallback != 0)
    ...
    if (status & SYS_STATUS_TXFRS)  // Transmit Frame Sent
    {
        ...
        if(dw1000local.dwt_txcallback != 0)
...
void dwt_ntbled_init( void )
{
	// set up GPIO0 & 1 for output, general purpose
	uint8_t buf[2];

	// Read directions
    dwt_readfromdevice(GPIO_CTRL_ID,0x08,2,buf);
    // Write masks for GPIO0/1
    buf[0] |= ( (1<<4) | (1<<5) );
    // Write output to GPIO0/1
    buf[0] &= ~(0x03);
    dwt_writetodevice(GPIO_CTRL_ID,0x08,2,buf);
}

void dwt_ntbled_toggle_green( void )
{
	// read current value
	uint8_t buf[1];
	dwt_readfromdevice(GPIO_CTRL_ID,0x0C,1,buf);
	// mask GPIO 1
	buf[0] |= (1<<5);
	// toggle GPIO 1
	buf[0] ^= 2;
	dwt_writetodevice(GPIO_CTRL_ID,0x0C,1,buf);
}

void dwt_ntbled_toggle_red( void )
{
	// read current value
	uint8_t buf[1];
	dwt_readfromdevice(GPIO_CTRL_ID,0x0C,1,buf);
...
int dwt_rxenable(int delayed)
{
    ...
    dwt_write16bitoffsetreg(SYS_CTRL_ID,0x00,temp) ;
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @fn void dwt_setinterrupt( uint32_tbitmask, uint8_tenable)
 *
